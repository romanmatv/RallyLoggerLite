...
gpxParser.prototype.parse = function (gpxstring) {
    ...
    /*let extensions = this.xmlSource.querySelector('extensions');
    if(extensions != null){
        console.log('extensions', extensions)
        console.log('extensions.children', extensions.children)

        for (let i=0; i<extensions.children.length; i++){
            let elem = extensions.children[i];
            console.log('ext_name', elem.tagName)
            console.log('ext_val', elem.textContent)
        }
        
        extensions.childNodes.forEach(child=>{
    ...
    for (let idx in wpts){
        ...
        pt.extensions = this.getExtensions(wpt);
    ...
    for (let idx in rtes){
        ...
        for (let idxIn in rtepts){
            ...
            pt.extensions = this.getExtensions(trkpt);
...
/**
 * Функция чтения параметров extensions
 * @param  {Element} node - Parent DOM Element
 */
gpxParser.prototype.getExtensions = function(node){
    let exts = {};

    if (node){
        let extensions = node.querySelector('extensions')
        ...
        if (extensions){
            for (let i=0; i<extensions.children.length; i++){
                ...
                exts[elem.tagName] = elem.textContent;
            }
        }
    }
